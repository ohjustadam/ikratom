...
## Every campaign is actionable by everyone; out-of-state advocates PICK, not auto-blast (2026-06-22)

Owner policy: "all campaigns should be actionable by all users always, regardless of their state." We removed the `wrongState`/`wrongLocality` hard-block cards in `CampaignAction.tsx` and stopped gating on `campaigns.allow_non_residents` (the column stays as data but no longer gates). There is **no server-side residency check** (`logCampaignAction`/`sendCampaignViaGmail` only record an `is_non_resident` flag), so removing the client gate is sufficient. The non-obvious part is **who an out-of-state advocate emails**. For the 73 explicit-target campaigns and the 4 federal ones it's trivial (exact officials / the user's own delegation). For the 79 **role-based state** campaigns, `getUserLegislators()` returns the user's OWN reps — wrong officials for another state's bill — so `[slug]/page.tsx` now drops reps not in the campaign's state and routes the out-of-state user through `pickableStateReps` (the campaign-state officials) + the existing `StateRepPicker`. We deliberately do **NOT** auto-target an entire out-of-state legislature: a one-click Gmail blast to ~150 officials from the advocate's own account would torch their email reputation and is exactly the low-quality astroturf the anti-weaponization work guards against. In-state users are unaffected — still one-click to their two district reps. **Local campaigns** scope the same way but by locality, not just state (`getUserLegislators` matches the user's OWN city/county, so a same-state user in a different city must be filtered out and routed to the locality picker). The **wave fire path** (`waves/fire.ts`) mirrors this — out-of-area wave joiners resolve to zero targets and are skipped rather than blasted to their own wrong-state reps (waves fire unattended, so there's no interactive picker; per-user picked targets at join time is a future follow-up). `campaigns.allow_non_residents` is **no longer an action gate** — it now only controls notification reach in `notify_users_for_campaign` (whether out-of-state users get pinged about a new campaign). The server send actions now re-validate campaign membership server-side (#656, `src/modules/campaigns/scope.ts`): `logCampaignAction`/`sendCampaignViaGmail` drop any client-supplied target outside the campaign's scope, so a crafted request can't repurpose a campaign to reach arbitrary officials (legit non-resident sends — campaign-state officials — stay allowed; `legislatorInCampaignScope` mirrors this page's resolution). Don't "simplify" by auto-blasting the whole legislature, and don't re-add the residency gate.
