...
export default async function AdminCampaignsPage() {
  ...
  const { data: campaigns } = await supabase
    .from("campaigns")
    .select("id, slug, title, state, active, auto_generated, created_at")
            ...
            {(campaigns ?? []).map((c) => (
              <tr key={c.id} className={c.active ? "" : "opacity-50"}>
                ...
                <td className="p-3 max-w-md truncate">
                  {c.auto_generated ? (
                    <span
                      title="Auto-generated by the bill-sync pipeline"
                      className="mr-1.5 inline-block rounded bg-sky-950/40 px-1.5 py-0.5 align-middle text-[10px] text-sky-300"
                    >
                      🤖 auto
                    </span>
                  ) : (
                    <span
                      title="Manually authored by an admin or advocate-leader"
                      className="mr-1.5 inline-block rounded bg-zinc-900 px-1.5 py-0.5 align-middle text-[10px] text-zinc-400"
                    >
                      👤 manual
                    </span>
                  )}
