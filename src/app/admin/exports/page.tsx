...
const EXPORTS = [
  { type: "campaigns", label: "Campaigns", desc: "All campaigns with state, scope, status, review_state. Useful for partner reporting." },
  { type: "actions", label: "Campaign actions", desc: "Every email/call ever logged. Method, timestamp, referrer (which embed widget drove it)." },
  { type: "advocates", label: "Advocates", desc: "All registered users (no email — IDs + matched districts + role flags + streak data). Use for partnership reach analysis." },
  { type: "bills", label: "Bills", desc: "Every active bill we track with stance, deep-analysis flags, signed/effective dates, official source URL." },
] as const;
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <p className="mt-6 rounded-md border border-zinc-800 bg-zinc-950/40 p-3 text-xs text-zinc-500">
        ...
        — anyone else gets a 403. Action logs in /admin/audit if you want a
        record of who pulled what.
      </p>
    </div>
  );
}
