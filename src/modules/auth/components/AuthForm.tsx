...
export function AuthForm({ redirectTo }: { redirectTo?: string }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (result?.error) {
        ...
        if ("hint" in result && result.hint) setErrorHint(result.hint);
      } else if (mode === "signup")
        setSuccess(
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      {error && (
        <div className="rounded-md border border-red-900/40 bg-red-950/40 px-3 py-2 text-sm">
          <p className="font-semibold text-red-300">{error}</p>
          {errorHint && (
            <p className="mt-1 text-xs text-red-200/80">{errorHint}</p>
          )}
          {mode === "signin" && (
            <div className="mt-2 flex flex-wrap gap-3 text-xs">
              <a href="/forgot" className="text-emerald-400 hover:underline">
                Forgot password →
              </a>
              <button
                type="button"
                onClick={() => { setMode("signup"); setError(null); setErrorHint(null); }}
