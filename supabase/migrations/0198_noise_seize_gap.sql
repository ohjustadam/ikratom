-- 0198_noise_seize_gap.sql
-- CREATE OR REPLACE auto_campaign_on_alert() (latest body = 0197) with ONE
-- one-token fix; everything else is verbatim from 0197.
--
-- NOISE-GAP FIX (mirrors scripts/lib/campaign-eligibility.mjs NOISE_RE). The
-- fda/dea eligibility gate's noise list enumerated `seiz|seizes|seizing|seized|
-- seizure`, but the surrounding word-boundary made the bare `seiz` dead and the
-- BASE verb "seize" matched nothing — so a present-tense seizure headline ("Feds
-- seize 7-OH from warehouse") escaped the noise filter and (with no ban verb)
-- fell through to escalate instead of being recognized as enforcement news. The
-- enumeration is replaced with the prefix `seiz[a-z]*`, which covers seize /
-- seizes / seizing / seized / seizure in one. Kept in lockstep with NOISE_RE.
--
-- Rollback: re-apply the 0197 function body.
